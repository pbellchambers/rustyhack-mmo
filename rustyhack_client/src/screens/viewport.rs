    ...
    for (name, position) in updates {
            ...
            && position.current_map == player.position.current_map
        {
            let relative_entity_position = RelativePosition {
                x: position.pos_x as i32 - viewport.viewable_map_top_left_position.x,
                y: position.pos_y as i32 - viewport.viewable_map_top_left_position.y,
            };
...
fn calculate_viewable_map_coords(viewport: &mut Viewport, player: &Player) {
    ...
    let x_view_distance = (viewport.width / 2) as i32;
    let y_view_distance = (viewport.height / 2) as i32;
    viewport.viewable_map_top_left_position = RelativePosition {
        x: player.position.pos_x as i32 - x_view_distance,
        y: player.position.pos_y as i32 - y_view_distance,
    }
}
            ...
            if (current_map_print_loc.x >= 0)
                ...
                && ((current_map_print_loc.x as usize)
                    < world_map
                        ...
                        .len())
                && ((current_map_print_loc.y as usize) < world_map.data().len())
            {
                screen.print(
